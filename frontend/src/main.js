...
function showLogin() {
  ...
  `);
    app.querySelectorAll("button[data-provider]").forEach((btn) => {
        btn.onclick = async () => {
            try {
                await login(btn.dataset.provider);
                ...
                if (container)
                    container.textContent = error instanceof Error ? error.message : "Error de autenticación";
            }
        };
    });
}
function showUsernameSetup() {
  ...
  `);
    const button = app.querySelector("#saveUsername");
    const input = app.querySelector("#username");
    if (!button || !input)
        return;
    button.onclick = async () => {
        try {
            ...
            const container = document.querySelector("#error");
            if (container)
                container.textContent = error instanceof Error ? error.message : "No se pudo guardar";
        }
    };
}
function showMenu() {
  ...
  `);
    app.querySelector("#play").onclick = () => showMissions(true);
    app.querySelector("#missions").onclick = () => showMissions(false);
    app.querySelector("#profile").onclick = async () => {
        const me = await getMe();
...
async function showMissions(autoStart) {
    ...
    `);
        ...
        app.querySelector("#start").onclick = () => startMission(m);
...
async function startMission(mission) {
    if (mission.status === "LOCKED") {
        ...
        if (lockMessage)
            lockMessage.textContent = "MISSION LOCKED: Complete Mission 01 to unlock this mission.";
        return;
    }
    if (mission.id !== 1) {
    ...
    const canvas = app.querySelector("#renderCanvas");
    ...
    function renderHud() {
      ...
      <div id="result"></div>
    `;
        hud.querySelector("#launchBtn").onclick = () => {
            if (launched || countdownTimer)
                return;
        ...
        hud.querySelector("#exitBtn").onclick = () => {
    ...
    engine.runRenderLoop(async () => {
        ...
        if (launched && position.y > 40) {
            ...
            if (result)
                result.textContent = "RESULTADO: MISIÓN COMPLETADA. Mission 02 desbloqueada.";
        }
        scene.render();
    });
